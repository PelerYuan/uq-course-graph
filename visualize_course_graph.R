...
if (!exists(".sourced_as_library")) {
  g <- readRDS("graph_object.rds")
  
  # 按你实际的选课情况改这两行;都留空的话图上就只有灰白配色
  completed_courses <- c("ENGG1100", "MATH1051", "ENGG1300", "CSSE1001")
  current_courses   <- c("CSSE2002", "CSSE2010", "CSSE2310", "MATH1052")
  
  g <- tag_course_status(g, completed_courses, current_courses)
  
  # 例1: 修完 CSSE1001 能解锁哪些课(下游一路到底)
  g %>%
    select_descendants("CSSE1001") %>%
    plot_course_graph(
      output_file = "subgraph_csse1001_unlocks.png",
      title = "Courses Unlocked by CSSE1001",
      width = 14, height = 10
    )
  
  # 例2: 修 CSSE4010 之前要先过哪几关(上游前置,只看2层)
  g %>%
    select_ancestors("CSSE4010", depth = 2) %>%
    plot_course_graph(
      output_file = "subgraph_csse4010_prereqs.png",
      title = "CSSE4010 Prerequisites (2 levels up)",
      width = 10, height = 8
    )
  
  # 例3: CSSE2310 和 CSSE2010 的关联网络,同时突出 ELEC/ENGG 学科的课程
  g %>%
    select_neighborhood(c("CSSE2310", "CSSE2010"), depth = 2) %>%
    plot_course_graph(
      output_file = "subgraph_csse_with_elec_highlight.png",
      title = "CSSE2310 & CSSE2010 Network (ELEC/ENGG highlighted)",
      highlight_prefix = c("ELEC", "ENGG"),
      width = 12, height = 9
    )
  
  # 例4: 只看 ELEC 学科内部的依赖链(硬过滤,其他学科完全移除)
  g %>%
    select_by_prefix(c("ELEC")) %>%
    plot_course_graph(
      output_file = "subgraph_elec_only.png",
      title = "ELEC Courses Only",
      width = 10, height = 8
    )
}